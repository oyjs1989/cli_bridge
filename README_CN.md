...
![Telegram 流式输出演示](https://github.com/kai648846760/cli-bridge/raw/master/testcase/Lark20260225-200437.gif)

### 钉钉 AI Card 流式输出

![钉钉 AI Card 流式输出演示](https://github.com/kai648846760/cli-bridge/raw/master/testcase/Lark20260225-200423.gif)

## 📋 前置要求

### 1. 安装 iflow CLI

cli-bridge 依赖 iflow CLI 运行，请先安装：
...
| `workspace` | string | `~/.cli-bridge/workspace` | 工作空间路径 |
...
# 添加间隔任务
cli-bridge cron add -n "喝水提醒" -m "该喝水了！" -e 300 -d --channel telegram --to "123456"

# 添加一次性任务
cli-bridge cron add -n "会议提醒" -m "开会！" -a "2024-12-25T10:00:00" -d --channel telegram --to "123456"

# 添加 cron 表达式任务
cli-bridge cron add -n "早报" -m "发送早报" -c "0 9 * * *" -d --channel telegram --to "123456"
...
cli-bridge 支持实时流式输出，让用户看到 AI "打字"的过程。
...
cli-bridge 自动管理多用户会话，支持跨渠道对话上下文。

**Session 映射存储**：
- 存储位置：`~/.cli-bridge/session_mappings.json`
